...
class MeanSquaredMedianDistance(Distance):
    ...
    def _calculate(self):
        ...
        diffmse = self.other - self.reference
        value_dot = np.vdot(diffmse, diffmse)

        valuemse = value_dot / f2
        valuemed = np.median(value_dot / f1).astype(np.float64)

        # diffmedian = diffmse / (max_ - min_)
        # valuemedian = np.median(np.abs(diffmedian)).astype(np.float64)

        value = (valuemse + valuemed)/2.0

        # calculate the gradient only when needed
        self._g_diff_mse = diffmse
        self._g_diff_median = diffmse
        self._g_f1 = f1
        self._g_f2 = f2
        gradient = None
        print(value)
    ...
    def gradient(self):
        if self._gradient is None:
            self._gradient = self._g_diff_mse / (self._g_f2 / 2)
        return self._gradient

    def __str__(self):
        return 'normalized MSEMAE = {:.2e}'.format(self._value)

MSEMAE = MeanSquaredMedianDistance
