...
def create_fmodel():
    ...
    with sess.as_default():
        ...
        # new_logits_as_tensor = tf.onvert_to_tensor(new_logits, np.float32)
        dictionary = mapping()
        keys = np.array(list(dictionary.keys()))
        imagenet_indices = []
        for k in keys:
            imagenet_indices.extend(k)
        unordered_tinyimagenet_logits = tf.manip.gather_nd(logits,imagenet_indices)
