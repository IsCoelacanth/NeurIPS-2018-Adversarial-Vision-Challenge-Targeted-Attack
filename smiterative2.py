    ...
    def _run_one(self, a, positive_salience, epsilon, stepsize, iterations,
        ...
        for _ in range(iterations):
            ...
            if positive_salience is not None:
                ...
                noise_mask = and_with_pos.astype(np.uint8)
...
class L1L2ClippingMixin(object):
    def _clip_perturbation(self, a, perturbation, epsilon):
        # using mean to make range of epsilons comparable to Linf
        norm = (np.sqrt(np.mean(np.square(perturbation))) + np.mean(np.abs(perturbation)))/2
        norm = max(1e-12, norm)  # avoid divsion by zero
        min_, max_ = a.bounds()
        s = max_ - min_
        # clipping, i.e. only decreasing norm
...
class L1L2DistanceCheckMixin(object):
    def _check_distance(self, a):
        if not isinstance(a.distance, distances.MSEMAE):
            logging.warning('Running an attack that tries to minimize the'
                            ' L1L2 norm of the perturbation without'
                            ' specifying foolbox.distances.MSEMAE as'
                            ' the distance metric might lead to suboptimal'
                            ' results.')
...
class SAIterativeAttack(
    ...
    @call_decorator
    def __call__(self, input_or_adv, pos_salience=None, label=None, unpack=True,
                 binary_search=True,
                 epsilon=0.3,
                 stepsize=0.06,
                 iterations=10,
        ...
        self._run(a, pos_salience, binary_search,
                  epsilon, stepsize, iterations,
                  random_start, return_early)
...
class NAGIterativeAttack(
    ...
    def _gradient(self, a, x, class_, strict=True):
        # get current gradient
        min_, max_ = a.bounds()
        y = x - self._decay_factor * self._momentum_history
        y = y.clip(min = min_, max = max_)
        gradient = a.gradient(y, class_, strict=strict)
        gradient = gradient / max(1e-12, np.mean(np.abs(gradient)))
    ...
    def _run_one(self, *args, **kwargs):
        ...
        # gradient descent
        self._momentum_history = 0
        return super(NAGIterativeAttack, self)._run_one(*args, **kwargs)
...
class NadamIterativeAttack(
    ...
    def _gradient(self, a, x, class_, strict=True):
        # get current gradient
        min_, max_ = a.bounds()
        y = x - self._decay_factor * self._gradient_history
        y = y.clip(min = min_, max = max_)
        noise = a.gradient(y, class_, strict=strict)
        noise = noise / max(1e-12, np.mean(np.abs(noise)))
        momentum = self._beta1 * self._gradient_history + (1 - self._beta1) * noise
        loss = self._beta2 * self._squared_gradient_history + (1 - self._beta2) * noise * noise
        self._gradient_history  = momentum
    ...
    def _run_one(self, *args, **kwargs):
        ...
        self._gradient_history = 0
        self._squared_gradient_history = 0
        return super(NadamIterativeAttack, self)._run_one(*args, **kwargs)
